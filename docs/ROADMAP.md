...
- [ ] **Single source-of-truth repo metadata** — drive the brittle README / model-card *facts* (test count, dashboard tab count, dataset & session counts, model status, artifact fidelity) from one machine-readable file and generate the prose from it (extend `scripts/generate_results.py`), so they stop drifting. Trigger: the project tree + these counts have re-staled repeatedly (2026-06-13 doc sweep). The directory *tree* is the lowest-value piece — generate the numbers first.
