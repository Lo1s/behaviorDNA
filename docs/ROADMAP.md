...
| 6.1. [Contrastive embeddings for identity](VERIFICATION.md#phase-61--contrastive-embeddings-for-identity-does-the-82-lever-transfer) | Does the Phase-8.2 contrastive lever transfer to identity? Frozen-embedding Balabit verification (cosine + classifier) + a scale-aug ablation | ✅ Done (2026-06-15) — **clean null**: contrastive embeddings tie *random projections* (~0.25 session-EER) and lose ~2× to hand-features (0.136); scale-ablation makes no difference; cross-domain (8.2) worse than random. Augmentation-contrastive ≠ user-discrimination → supervised/metric contrastive is the open lever. [docs/VERIFICATION.md](VERIFICATION.md#phase-61--contrastive-embeddings-for-identity-does-the-82-lever-transfer) |
...
- [x] REPORT.md §6 — full section (Balabit EER + SapiMouse users-curve + the §6.1 contrastive-identity null), 2026-06-15
...
- [x] **dual-capture ingest pipeline** (2026-06-15) — `pipeline/outcome/dual_capture.py` (+ CLI `scripts/ingest_dual_capture.py`) turns a recorder JSON + `.dem` into one **clock-synced, window-joined** table (input features ⨝ outcome features on `(session_id, window_idx)`), reusing the production feature path verbatim and correcting the xcorr offset back to the window anchor so the two grids align exactly. `tests/test_dual_capture.py` (8 tests, incl. a real-demo integration). Validated end-to-end on the public demo (`peak_corr ≈ 1.0`, 31 windows joined). This is the entrypoint real sessions run through; only the detector below still needs the data.
- [ ] supervised detection benchmark (classifier head) + Phase 4.1 aggregator re-attempt — **awaits dual-capture data** (recorder + demo recorded simultaneously; cheat positives need offline `cheat_sim` capture).
