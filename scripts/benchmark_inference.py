...
# considered production-trustworthy. The current LightGBM→ONNX export fails this
# (onnxmltools 1.16 / lightgbm 4.6 multiclass-probability bug) — see docs/FINDINGS.md.
...
def predict_labels_onnx(session, X_np: np.ndarray) -> np.ndarray:
    """ONNX Runtime path on raw float32 features (scaler is baked into the graph)."""
    name = session.get_inputs()[0].name
    out = session.run(["label"], {name: np.ascontiguousarray(X_np, dtype=np.float32)})
...
def predict_proba_onnx(session, X_np: np.ndarray) -> np.ndarray:
    ...
    name = session.get_inputs()[0].name
    out = session.run(
        ["probabilities"], {name: np.ascontiguousarray(X_np, dtype=np.float32)}
    )
...
def _load_X(n_min: int = 256) -> np.ndarray:
...
def run(argv: list[str] | None = None) -> int:
    ...
    ox_rows = [X_f32[i : i + 1] for i in range(len(X_f32))]
    ...
    ox_tp = _throughput(
        lambda b: predict_labels_onnx(session, b), X_f32[: args.batch], args.reps
    )
    ...
    if not fidelity_ok:
        log.warning(
            "ONNX export is NOT production-trustworthy: probability MAE %.3f >> %.0e. "
            "The LightGBM→ONNX converter (onnxmltools 1.16 / lightgbm 4.6) mishandles "
            "multiclass probabilities. ONNX latencies are reference-only until a "
            "compatible converter or a sklearn-native model is used. See docs/FINDINGS.md.",
            prob_mae,
            FIDELITY_THRESHOLD,
        )
