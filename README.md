...
*Chunk-level cheat detection — **synthetic cheats injected into 22 real GTA legit sessions** (the approach proof). The LSTM autoencoder's reconstruction error separates cheat chunks (coloured) from legit-behaviour chunks (green): triggerbot ROC AUC 0.94, aimbot 0.80, macro 0.62 — while hand-crafted window features stay at chance for aimbot. The approach is **also validated on real recorded cheats and on a second game (CS2)** — see [Results](#results-at-a-glance). Reproduce with `python -m scripts.build_phase4_demo`. See [docs/ADVERSARIAL.md](docs/ADVERSARIAL.md) and [docs/STREAMING.md](docs/STREAMING.md).*

> **Data status.** Measured on real GTA gameplay (22 legit sessions, 4 players) + real **recorded** cheats (a controllable offline harness, [docs/CHEAT_DATA_COLLECTION.md](docs/CHEAT_DATA_COLLECTION.md)) + an **external** CS2 cheat dataset. The chunk-level detector works on real data and transfers across games; the *session-level* live-risk aggregator saturates on the current single-recorder calibration set and is gated pending more (cross-player) data (Phase 4.1 — see [docs/STREAMING.md](docs/STREAMING.md#what-works-and-what-doesnt-honest)).
...
- **End-to-end MLOps on *real* data:** custom Windows telemetry recorder → DVC pipeline → training → calibration → drift monitoring → MLflow model registry → FastAPI + ONNX serving + Streamlit dashboard. Every PR runs the unit suite (**510 tests**) + lint + the generated-doc/DVC-dependency gates; the full `dvc repro` runs on `main` (it needs the data secret) — see [CI](.github/workflows/ci.yml).
...
- **Honest validation over flattering numbers:** found, root-caused and **fixed a real ONNX serving-fidelity bug** (float32 precision flipping an overfit model's predictions — now a bit-faithful float64 export behind a CI regression gate, [finding #7](docs/FINDINGS.md)); *verified* (not assumed) a session-level detection ceiling before building on it; an **ablation** showing the model is over-parameterised at this N; an **architecture study** finding LSTM/TCN/Transformer statistically tied; every *identification* headline number ships with a **bootstrap CI** (cheat-detection chunk AUCs are point estimates, and the held-out classical benchmark adds repeated-split CIs — [docs/ADVERSARIAL.md](docs/ADVERSARIAL.md#held-out-base-session-benchmark-classical-features)).
- **Anti-cheat framing throughout:** false-positive/ban-cost reasoning, *measured* probability calibration (ECE/Brier; serving returns raw `predict_proba` — see the Model Card), and deliberate, audited model promotion — see the **[Model Card](MODEL_CARD.md)**.
...
**Player identification** (behavioural biometric, real GTA, per 30 s window — 95% CIs from a 2000-resample window bootstrap; intervals are wide because the test set is 53 windows):

| Setting | Result |
|---|---|
| 4 players (real GTA) | **0.72** acc (95% CI 0.60–0.83) · 0.73 F1 (95% CI 0.62–0.85) |
