...
| `cs2cd_full/` | **CS2CD full release** — 795 matches (478 `no_cheater_present` + 317 `with_cheater_present`, ~48 GB); player-**anonymised**, match-level labels (no `cheater_present` column) | Phase 8.1 in-domain pretraining — `pipeline/pretraining/cs2cd_full.py`, `scripts/indomain_transfer.py` | HF `CS2CD/CS2CD.Counter-Strike_2_Cheat_Detection` (auto-fetched by `python -m pipeline.pretraining.cs2cd_full --step all`) |
